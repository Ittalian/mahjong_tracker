...
import 'package:mahjong_tracker/services/gamble_member_service.dart';

// ──────────────────────────────────────────────────────
// グループ一覧・管理画面
// ──────────────────────────────────────────────────────
class PachinkoGroupListScreen extends StatefulWidget {
  const PachinkoGroupListScreen({super.key});

  @override
  State<PachinkoGroupListScreen> createState() => _PachinkoGroupListScreenState();
}

class _PachinkoGroupListScreenState extends State<PachinkoGroupListScreen> {
  final _service = PachinkoGroupService();

  @override
  Widget build(BuildContext context) {
    final typeColor = Colors.deepPurple.shade700;

    return Scaffold(
      appBar: AppBar(
        title: const Text('パチスログループ管理'),
      ),
      body: StreamBuilder<List<PachinkoGroup>>(
                            ...
                            child: _GroupCard(
                              ...
                              onEdit: () async {
                                await Navigator.push(
                                  ...
                                  MaterialPageRoute(
                                    builder: (_) =>
                                        PachinkoGroupEditScreen(group: group),
                                  ),
                                );
                              },
                              onDelete: () async {
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => const PachinkoGroupEditScreen(),
            ),
          );
        },
        tooltip: 'グループを追加',
...
// グループ追加・編集画面
// ──────────────────────────────────────────────────────
class PachinkoGroupEditScreen extends StatefulWidget {
  final PachinkoGroup? group;

  const PachinkoGroupEditScreen({super.key, this.group});

  @override
  State<PachinkoGroupEditScreen> createState() =>
      _PachinkoGroupEditScreenState();
}

class _PachinkoGroupEditScreenState extends State<PachinkoGroupEditScreen> {
  final _formKey = GlobalKey<FormState>();
  final _nameController = TextEditingController();
  final _newMemberController = TextEditingController();
  final _groupService = PachinkoGroupService();
  final _gambleMemberService = GambleMemberService();
          ...
          child: Column(
            ...
            children: [
              ...
              StreamBuilder<List<String>>(
                stream: _gambleMemberService.getUniqueMembers(),
                builder: (context, snapshot) {
              ...
              Row(
                children: [
                  ...
                  FilledButton.icon(
                    onPressed: () => _addMember(_newMemberController.text),
